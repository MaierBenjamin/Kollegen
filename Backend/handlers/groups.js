...
export async function getGroup(req, res) {
  ...
  const [[organizationUser]] = await safeOperation(
    () => db.query("select userRole from organization_users where fk_UserId = ? and fk_OrganizationId = ?", [req.session.user.id, group.fk_OrganizationId]),
    "Error while retrieving organization user from database"
  )
  ...
  const [channels] = await safeOperation(
    () => db.query("select groupChannelId as channelId, name from group_channels where fk_GroupId = ?", [groupId]),
...
export async function getChannelMessages(req, res) {
  ...
  const [[channel]] = await safeOperation(
    () => db.query(
      `select fk_OrganizationId from group_channels 
  ...
  const [[organizationUser]] = await safeOperation(
    () => db.query("select userRole from organization_users where fk_UserId = ? and fk_OrganizationId = ?", [req.session.user.id, channel.fk_OrganizationId]),
    "Error while retrieving organization user from database"
  )
  ...
  const [messages] = await safeOperation(
    () => db.query("select content as message, fk_UserId as userId from channel_message where fk_GroupChannelId = ?", [channelId]),
    "Error while updating group"
  )
...
export async function sendChannelMessage(req, res) {
  ...
  await safeOperation(
    () => db.query("insert into channel_message (content, fk_UserId, fk_GroupChannelId) values (?,?,?)", [message, req.session.user.id, channelId]),
...
export async function createChannel(req, res) {
  ...
  await safeOperation(
    () => db.query("insert into group_channels (name, fk_GroupId) values (?,?)", [channelName, groupId]),
...
export async function deleteChannel(req, res) {
  ...
  await safeOperation(
    () => db.query("delete from group_channels where groupChannelId = ?", [channelId]),
